...
/*   Updated: 2020/02/20 11:10:01 by juligonz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	get_perpendicular_dist(t_vector object, t_raycast *r)
{
	double result;
	...
	if (r->side == 1)
		...
		result = (object.y - g_game.cam.pos.y + (1 - r->step.y) / 2) / r->ray_dir.y;
	return (result);
}
...
t_vector	calculate_sprite_height(t_raycast *r, t_sprite sprite)
{
	...
	r->line_height = (int)(g_app.res.y / sprite.perp_dist_rel_player);
